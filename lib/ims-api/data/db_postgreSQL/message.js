...
var Message = {
  getFromDialog: function (dialog_id, callback) {
    let queryString = "SELECT message.message_id,message.body,message.sender,message.dialog,message.created_at, user_account.name as author_name, user_account.avatar as author_avatar FROM message INNER JOIN user_account ON user_account.user_id = message.sender WHERE message.dialog =" + dialog_id+ " ORDER BY message.created_at ASC";
  ...
  getLastInDialog: function (dialog, callback) {
    ...
    let queryString = "SELECT message.message_id,message.body,message.sender,message.dialog,message.created_at, user_account.name as author_name, user_account.avatar as author_avatar FROM message INNER JOIN user_account ON user_account.user_id = message.sender WHERE message.dialog =" + dialog_id+ " ORDER BY message.created_at DESC LIMIT 1";
    client.query(queryString, function(error, result) {
      if(error) {
        return callback(error, null);
      }
  ...
  getDialogs: function (user_id, callback) {
    let queryString = "SELECT DISTINCT contact_list.dialog_id, user_account.user_id, user_account.name, user_account.avatar FROM contact_list INNER JOIN user_account ON user_account.user_id=contact_list.contact_id INNER JOIN message ON message.dialog=contact_list.dialog_id WHERE contact_list.user_id=" + user_id;
    client.query(queryString, function(error, result) {
      if(error) {
        return callback(error, null);
      }
