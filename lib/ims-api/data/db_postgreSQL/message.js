...
var Message = {
  ...
  getDialogs: function (user_id, callback) {
    let queryString = "SELECT contact_list.dialog_id, user_account.user_id, user_account.username, user_account.first_name, user_account.last_name, user_account.avatar FROM contact_list INNER JOIN user_account ON user_account.user_id=contact_list.contact_id WHERE contact_list.user_id=" + user_id;
    client.query(queryString, function(error, result) {
      if(error) {
        return callback(error, null);
      }
