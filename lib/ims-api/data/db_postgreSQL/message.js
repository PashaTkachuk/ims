...
var Message = {
    ...
    getRooms: function (user_id) {
        let queryString =
            "SELECT DISTINCT room.room_id, room.name, room.last_message_id, room.last_updated_at, message.message_id, profile.avatar as sender_avatar, profile.name as author_name, message.body as last_message, message.created_at " +
            "FROM room " +
            "INNER JOIN message " +
            "ON message.message_id = room.last_message_id " +
            "INNER JOIN profile " +
            "ON message.sender = profile.user_id " +
