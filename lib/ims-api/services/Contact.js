...
var Contact = {
  addContact: function (user_id, contact_email, callback) {
    contactModel.add(user_id, contact_email, function (error, result) {
      if (error) {
        callback(error, null);
      } else {
  ...
  getRoom: function (first_user_id, second_user_id, callback) {
    contactModel.getRoom(first_user_id, second_user_id, function (error, dialog_id) {
      if (error) {
