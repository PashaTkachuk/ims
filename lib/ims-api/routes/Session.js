...
var SessionRouter = {
	createSession: function (sock, args, next) {
		var body = args[1];
		sessionService.addSession(body.email, body.password, sock.id)
	...
	deleteSession: function (sock, args, next) {
		var body = args[1];
		sessionService.deleteSession(body.user_id)
			.then(function (result) {
 	...
 	checkSession: function (sock, args, next) {
		var body = args[1];
 		sessionService.checkSession(body.user_id, body.sid, sock.id)
			.then(function (result) {
				if (!result.status){
