...
var Contact = {
  getContacts: function (user_id, callback) {
    let queryString = "SELECT contact_list.dialog_id, user_account.user_id, user_account.username, user_account.email, user_account.first_name, user_account.last_name, user_account.avatar, user_account.skype, user_account.phone FROM user_account, contact_list WHERE contact_list.contact_id = user_account.user_id AND contact_list.user_id =" + user_id;
  ...
  addContact: function (user_id, contact_username, callback) {
    var self =this;
    let queryString = "SELECT user_account.user_id FROM user_account WHERE user_account.username =\'" + contact_username + "\'";
    client.query(queryString, function(error, result) {
      if(error) {
        return callback(error, null);
      }
