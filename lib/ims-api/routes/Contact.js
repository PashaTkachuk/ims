...
var ContactRouter = {
	addContact: function (sock, args, next) {
		...
		contactService.addContact(body.user_id, body.email)
			.then(function (result) {
				sock.emit('add to roster feedback', {status: true,  message: 'Contact successful added.'});
			})
			.catch(function (error) {
				sock.emit('add to roster feedback', {status: false, message: error.message});
			});
	},
	...
	createRoom: function (sock, args, next) {
		var body = args[1];
		contactService.createRoom(body.user_id, body.contact_id)
			.then(function (room_id) {
				sock.emit('create room feedback', {status: true, room_id: room_id});
			})
			.catch(function (error) {
				sock.emit('create room feedback', {status: false});
			});
	}
};
