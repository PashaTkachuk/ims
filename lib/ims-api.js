...
var messageService = require('./ims-api/services/Message');
var userService = require('./ims-api/services/User');
...
var storage = multer.diskStorage({
	destination: function (req, file, cb) {
		cb(null, 'media/images/profile/')
	},
	filename: function (req, file, cb) {
		cb(null, 'avatar_user_' + req.cookies.user_id);
	}
});
...
router.post('/user/avatar', upload.single('avatar'), user_router.updateAvatar);
...
io.on('connection', function(socket) {
	socket.on('change status', function(data){
		if (data.status = 'online') {
			clients.push({user_id: data.user_id, socket_id: socket.id});
		}

		userService.updateUser(data.user_id, {status: data.status}, function (error, result) {
			if (error) {
				return;
			} else {
				io.emit('change status', {user_id: data.user_id, status: data.status});
	...
	socket.on('switch room', function(data){
		...
		console.log('user joined ' + data.newRoom + ' dialog');
	});

	socket.on('message', function(message){
		messageService.addMessage(message, function (error, result) {
			if (error) {

			} else {
				io.in(message.dialog).emit('message', message);
				console.log('Send message: body=>' + message.body + "   by=> " + message.user_id);
			}
		});
	});
	...
	socket.on('disconnect', function(){
		...
		userService.updateUser(user_id, {status: 'offline'}, function (error, result) {
			if (error) {
				return;
			} else {
				io.emit('change status', {user_id: user_id, status: 'offline'});
